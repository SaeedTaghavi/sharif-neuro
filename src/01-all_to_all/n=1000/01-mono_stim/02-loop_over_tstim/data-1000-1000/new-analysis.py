...
#    temp_string=r'$\sin(\theta_i)$,  i $\in$ {1,...,'+temp_string+'}'
    ...
    theta_befor_stim = theta_befor_stim % (2*pi)
#    plt.hist(theta0, bins=np.arange(min(theta0),max(theta0),.1), bottom=10, color="black")
    n, binss, patches=plt.hist(theta_befor_stim, bins=np.arange(min(theta_befor_stim),max(theta_befor_stim),.05), bottom=100, color="blue",alpha=.5)
#    n, bins, patches = plt.hist(theta_befor_stim,bins=np.arange(min(theta_after_stim),max(theta_after_stim),0.025),alpha=0.7,color="blue")
    theta_after_stim = theta_after_stim % (2*pi)
#    plt.hist(theta_after_stim,bins=bins,alpha=0.7,color="red",bottom=10)
    plt.hist(theta_after_stim, bins=np.arange(min(theta_after_stim),max(theta_after_stim),.05), bottom=100, color="red",alpha=.5)
    plt.xticks([])
    plt.yticks([])
    filename='theta_tp={:08.4}'.format(it_stim*dt)
    extension="_.png"
